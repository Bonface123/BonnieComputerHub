...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_user'])) {
    ...
    try {
        $update_sql = "UPDATE users SET name = ?, email = ?, role = ?, avatar = ? WHERE id = ?";
        $stmt = $pdo->prepare($update_sql);
        $stmt->execute([$name, $email, $role, $avatar, $user_id]);
        ...
        <div class="max-w-2xl mx-auto bg-white rounded-lg shadow-md p-6">
            ...
            <form action="" method="POST" enctype="multipart/form-data" class="space-y-6">
                <div class="mb-4">
                    ...
                    <img src="get_avatar.php?id=<?= $user['id'] ?>" alt="Avatar" class="w-16 h-16 rounded-full border mb-2">
                    <input type="file" name="avatar" accept="image/*" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-primary">
                </div>
                <div>
